    ...
    async (req, res) => {
        if(req.method === "GET" && req.url === "/api/users"){ // GetAllUsers API
            ...
            req.on('end', async () => {
                ...
                const {title, author, price} = JSON.parse(books);
                await book.create(title, author, price);
                console.log('succcccccccccccccccedd');
            })
        ...
        }else if(req.method === "POST" && req.url === "/api/users"){ // Add or RegisterUser User API
            let data = "";
            req.on('data', (chunk) => {
                data += chunk.toString()
            })
            req.on('end', async () => {
                ...
                //     })
                // }
                const { email, name, username } = JSON.parse(data);
                await user.create(email, name, username);
                console.log('succccccccccccess');
            })
